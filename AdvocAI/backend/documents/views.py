...
from ai_generator.utils import get_gemini_response, extract_json_from_response # Import the AI generation function and helper
...
def create_conversation_with_chat(request):
    ...
    try:
        ...
        ai_response_content = ""
        document_data = extract_json_from_response(ai_raw_response)
        if document_data:
            ai_response_content = document_data.get('text', '')
        else:
            # If not JSON, it's just a chat message, so no document content yet
            ai_response_content = ""
        ...
        # If we generated a document, don't show the raw JSON in the chat.
        # Instead, show a friendly message.
        chat_response_text = ai_raw_response
        if ai_response_content:
            chat_response_text = "I have drafted the document for you. You can see it in the document panel."

        messages = [
            {'sender': 'user', 'text': message},
            {'sender': 'bot', 'text': chat_response_text} 
        ]
        ...
        if conversation_id:
            return Response({
                ...
                'response': chat_response_text, # Return the friendly text, not the raw JSON
...
def send_chat_message(request, pk):
    ...
    try:
        ...
        ai_raw_response = get_gemini_response(message, document_content, conversation_history)

        # Parse the AI response to extract document content
        ai_response_content = None # Initialize as None to indicate no change if no JSON
        document_data = extract_json_from_response(ai_raw_response)
        if document_data:
            ai_response_content = document_data.get('text', '')
        else:
            # If not JSON, it's just a chat message, so no new document version
        ...
        chat_response_text = ai_raw_response
        if ai_response_content:
            chat_response_text = "I have updated the document based on your request."

        updated_messages = conversation.get('messages', [])
        updated_messages.append({'sender': 'user', 'text': message})
        updated_messages.append({'sender': 'bot', 'text': chat_response_text})
        ...
        success = update_conversation(
            ...
            updated_messages,
            ai_response_content, # AI's parsed response is the new document content (or None)
            uploaded_by=(request.user.username if request.user.is_authenticated else 'anonymous'),
            notes='Document update via chat message'
        )

        if success:
            # If ai_response_content is None, return the OLD content to the frontend
            # so it doesn't blank out the view or show nothing.
            content_to_return = ai_response_content if ai_response_content is not None else document_content
            
            return Response({
                'response': chat_response_text, # Return the friendly text
                'updated_document_content': content_to_return
